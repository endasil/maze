...
# if UNITY_EDITOR
		...
		protected virtual IEnumerator PostProcess()
		{
			...
			if (!IsAnalysis)
			{
				...
				foreach (var tile in CurrentDungeon.AllTiles)
				{
					...
					ProcessProps(tile, tile.gameObject);
				}
		...
		{
			foreach (var prop in root.GetComponents<RandomProp>())
				prop.Process(RandomStream, tile);

			// Recursively process props so they can be nested
			int childCount = root.transform.childCount;
			for (int i = 0; i < childCount; i++)
			{
				var child = root.transform.GetChild(i).gameObject;
				ProcessProps(tile, child);
			}
		}
