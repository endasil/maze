...
namespace DunGen
{
	...
	public class AdjacentRoomCulling : MonoBehaviour
	{
		...
		protected void RefreshDoorVisibilities()
		{
			foreach (var door in allDoors)
			{
		...
		protected void SetDoorVisibility(Door door, bool visible)
		{
		...
		protected void UpdateVisibleTiles()
		{
			visibleTiles.Clear();
		...
		protected void SetTileVisibility(Tile tile, bool visible)
		{
			// Renderers
			List<Renderer> renderers;
		...
		public void UpdateRendererLists()
		{
			foreach (var tile in allTiles)
			{
				// Renderers
		...
		private Tile SearchForNewCurrentTile()
		{
